...
class Node(rclpy.node.Node):
    ...
    **kwargs :
    ...
    def __init__(
        self, *args, node_name="node", namespace="machine", machine=None, **kwargs
    ):
        super().__init__(*args, node_name=node_name, namespace=namespace, **kwargs)
    ...
    def initialize_publishers(self):
        ...
        * ``machine/event``
        ...
        kwargs = dict(topic="state", msg_type=state_message, qos_profile=DEFAULT_QOS)
        self.state_publisher = self.create_publisher(**kwargs)

        # Create a publisher for reporting task events / triggers.
        kwargs = dict(topic="event", msg_type=state_message, qos_profile=DEFAULT_QOS)
    ...
    def log_state_change(self, event_data):
        ...
        * The *new* state is published to the ``state`` ROS topic.
        ...
        if not is_internal:
            self.state_publisher.publish(msg=message)
