...
if __name__ == "__main__":
    ...
    class Model:
        ...
        def on_exit_active(self, event_data):
            stop_timer()

        def on_enter_inactive(self, event_data):
            self.activate()
    ...
    state_transitions = [{**r, "before": "log_timestamp"} for r in state_transitions]
    ...
    machine = transitions.Machine(
        model=model, states=states, transitions=state_transitions, initial="inactive"
    )
    ...
    def event_callback(message):
        ...
        sample_buffer[index] = (
            client.get_clock().now().nanoseconds if message.data == "timeout" else None
        )
        if all(sample_buffer):
            future.set_result(sample_buffer)
    ...
    timestamps_inactive_ns = [
        r["timestamp"] for r in timestamp_records if (r["state"] == "inactive")
    ]
